...
export default function VentasPage() {
  ...
  async function handleSubmit(e) {
    e.preventDefault();
    if (items.length === 0 || items.some((it) => !it.producto_id)) {
    ...
    // 2. Crear items de venta + descontar stock + registrar movimiento
    for (const it of items) {
      ...
      const nuevoStock = (prod?.stock || 0) - Number(it.cantidad);
      await supabase.from("productos").update({ stock: nuevoStock }).eq("id", it.producto_id);
            ...
            <div className="mb-3">
              ...
              {items.map((it, idx) => (
                <div key={idx} className="grid grid-cols-12 gap-2 mb-2 items-center">
                  ...
                  />
                  <input
                    type="number" step="0.01" className="input-field col-span-3"
